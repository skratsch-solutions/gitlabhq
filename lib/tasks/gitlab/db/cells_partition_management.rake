...
namespace :gitlab do
  namespace :db do
    # @return [String]
    #   JSON array of { database, tables: [{ table_name, partitions: [{ partition_name, from, to }] }] }.
    desc 'GitLab | Cells | DB | Export integer range partition definitions from all databases as JSON'
    ...
    desc 'GitLab | Cells | DB | Ensure integer range partitions exist on all databases from exported definitions file'
