  ...
  describe 'next_partition_if callback' do
    ...
    context 'when the partition has records' do
      before do
        described_class.create!(
          fully_qualified_table_name: 'public.table', primary_key_value: 1, status: :processed, **sharding_key
        )
        described_class.create!(
          fully_qualified_table_name: 'public.table', primary_key_value: 2, status: :pending, **sharding_key
        )
      end
    ...
    context 'when the first record of the partition is older than PARTITION_DURATION' do
      before do
        ...
        described_class.create!(fully_qualified_table_name: 'public.table', primary_key_value: 2, **sharding_key)
      end
  ...
  describe 'detach_partition_if callback' do
    ...
    context 'when the partition contains only processed records' do
      before do
        ...
        described_class.create!(
          fully_qualified_table_name: 'public.table', primary_key_value: 2, status: :processed, **sharding_key
        )
      end
  ...
  describe 'the behavior of the strategy' do
    it 'moves records to new partitions as time passes', :freeze_time do
      Gitlab::Database::SharedModel.using_connection(ApplicationRecord.connection) do
        ...
        described_class.create!(fully_qualified_table_name: 'public.table', primary_key_value: 1, **sharding_key)
        ...
        expect do
          described_class.create!(fully_qualified_table_name: table, primary_key_value: 5, **sharding_key)
