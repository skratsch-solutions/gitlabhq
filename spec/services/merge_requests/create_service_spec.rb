...
RSpec.describe MergeRequests::CreateService, :clean_gitlab_redis_shared_state, feature_category: :code_review_workflow do
  ...
  describe '#execute' do
    ...
    context 'with early pipeline creation request' do
      let(:opts) do
        {
          title: 'Awesome merge_request',
          source_branch: 'feature',
      ...
      context 'when track_ref_pipeline_creation feature flag is enabled' do
        ...
        it 'creates a pipeline creation request and passes it to NewMergeRequestWorker' do
          expect(Ci::PipelineCreation::Requests).to receive(:start_for_merge_request)
            .and_call_original

          expect(NewMergeRequestWorker).to receive(:perform_async)
            .with(anything, user.id, hash_including('pipeline_creation_request'))

          described_class.new(project: project, current_user: user, params: opts).execute
        end

        it 'makes pipeline_creating_for_merge_request? return true' do
          merge_request = nil

          expect(NewMergeRequestWorker).to receive(:perform_async) do |mr_id, _user_id, params|
            merge_request = MergeRequest.find(mr_id)
            expect(params['pipeline_creation_request']).to be_present
            expect(Ci::PipelineCreation::Requests.pipeline_creating_for_merge_request?(merge_request)).to be true
          end
