...
RSpec.describe Authn::IamService::RejectConsentChallengeService, feature_category: :system_access do
  ...
  describe '#execute' do
    let(:response) { ::Auth::RejectConsentChallengeResponse.new(redirect_to: redirect_url) }
    ...
    context 'when redirect_to is missing' do
      let(:response) { ::Auth::RejectConsentChallengeResponse.new(redirect_to: '') }
    ...
    context 'when redirect_to is an untrusted URL' do
      let(:response) { ::Auth::RejectConsentChallengeResponse.new(redirect_to: 'https://untrusted.example.com/oauth2/authorize') }
