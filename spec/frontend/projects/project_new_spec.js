...
describe('New Project', () => {
  const autoGeneratedSlugClass = '!gl-bg-feedback-info';
  ...
  describe('deriveSlugFromProjectName', () => {
    ...
    it('adds auto-generated class to the project path input', () => {
      $projectName.value = 'My Awesome Project';

      projectNew.onProjectNameChange($projectName, $projectPath);

      expect($projectPath.classList.contains(autoGeneratedSlugClass)).toBe(true);
    });
  });

  describe('auto-generated slug class', () => {
    beforeEach(() => {
      projectNew.bindEvents();
    });

    it('is added when the project name changes', () => {
      $projectName.value = 'My Awesome Project';
      mockChange($projectName);

      expect($projectPath.classList.contains(autoGeneratedSlugClass)).toBe(true);
    });

    it('is removed when the user manually edits the project path', () => {
      $projectName.value = 'My Awesome Project';
      mockChange($projectName);
      expect($projectPath.classList.contains(autoGeneratedSlugClass)).toBe(true);

      $projectPath.value = 'custom-slug';
      mockChange($projectPath);

      expect($projectPath.classList.contains(autoGeneratedSlugClass)).toBe(false);
    });

    it('remains when the project name is cleared', () => {
      $projectName.value = 'My Awesome Project';
      mockChange($projectName);
