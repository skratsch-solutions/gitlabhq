...
| [`sub`](https://www.rfc-editor.org/rfc/rfc7519.html#section-4.1.2) | Subject of the token ("subject" claim). Defaults to `project_path:{group}/{project}:ref_type:{type}:ref:{branch_name}`. Can be configured for the project with the [projects API](../../api/projects.md#update-a-project). The `sub` claim can include additional fields such as `ref_protected`, and environment-related fields such as `environment_protected` and `deployment_tier` when jobs specify an environment. Introduced in GitLab 18.7. |
