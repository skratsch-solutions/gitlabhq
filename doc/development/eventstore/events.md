...
| `MergeRequests::ClosedEvent` | `code_review_workflow` | EE | Published when a merge request is closed, so EE subscribers (such as security policy and AutoFlow workers) can react to the state change. |
| `MergeRequests::CreatedEvent` | `code_review_workflow` | EE | Published when a new merge request is created and prepared, so EE subscribers (such as Cluster Agent AutoFlow workers) can react to the new merge request. |
...
| `MergeRequests::MergedEvent` | `code_review_workflow` | EE | Published when a merge request has been merged and post-merge processing runs, allowing EE subscribers (such as compliance, security policy, AutoFlow, and audit workers) to react. |
...
| `MergeRequests::ReopenedEvent` | `code_review_workflow` | EE | Published when a closed merge request is reopened, so EE subscribers (such as Cluster Agent AutoFlow workers) can react to the state change. |
...
| `MergeRequests::UpdatedEvent` | `code_review_workflow` | EE | Published when a merge request is updated, so EE subscribers (such as Cluster Agent AutoFlow workers) can react to changes on the merge request. |
