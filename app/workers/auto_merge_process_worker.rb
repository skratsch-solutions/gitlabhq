...
class AutoMergeProcessWorker # rubocop:disable Scalability/IdempotentWorker
  ...
  def perform(params = {})
    ...
    use_primary_db_reads if Feature.enabled?(:auto_merge_use_primary_db, merge_requests.first.project)
  ...
  # Force reads to the primary so mergeability decisions are never made
  # against a stale replica. Same operation that `data_consistency :always`
  # triggers in the Sidekiq server middleware. See gitlab-org/gitlab#596177.
  def use_primary_db_reads
