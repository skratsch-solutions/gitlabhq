  ...
  module PrinciplesDistiller
    class Sync
      module AutoMr
        ...
        SKILL.md files, and the Duo review-instructions fences from the
        principles manifest.
        ...
        `CLAUDE.md`, and the `gitlab-coding-principles` SKILL.md files. It also
        refreshes the generated fences in
        `.gitlab/duo/mr-review-instructions.yaml` from the distilled principles.

        These files embed the routing table for **all** principles (or, for the
        Duo review-instructions file, generated fences imported from the
        distilled principles), so they are kept separate from the per-team
        principle MRs in this run. The Duo file's CODEOWNERS routing is left to
        the broad `/.gitlab/` rule.
        #{job_line}
        No distilled principle content changes here — only the generated
        routing tables and Duo review fences.
          DESC
