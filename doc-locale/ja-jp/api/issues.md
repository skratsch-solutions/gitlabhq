...
このAPIを使用して、[イシュー](../user/project/issues/_index.md)を管理します。次のことができます: 

- イシューを作成、更新、および削除します。
- 担当者、ラベル、マイルストーン、タイムトラッキングなどのイシューメタデータを管理します。
- イシューとマージリクエストを相互参照します。
- プロジェクトとエピック間のイシューの移動とプロモートを追跡します。
- 認可チェックでアクセスと表示レベルを制御します。
...
認証済みユーザーがアクセスできるすべてのイシューをリスト表示します。デフォルトでは、現在のユーザーが作成したイシューのみが返されます。すべてのイシューをリスト表示するには、パラメータ`scope=all`を使用します。
...
| `assignee_username`             | 文字列配列  | いいえ         | 指定された`username`に割り当てられているイシューを返します。`assignee_id`と類似しており、`assignee_id`と相互に排他的です。GitLab Community Edition（CE）では、`assignee_username`配列には単一値のみが含まれている必要があります。そうでない場合には、無効なパラメータエラーが返されます。渡されたすべてのユーザーに割り当てられたイシューのみが返されます。 |
...
## すべてのグループイシューをリスト表示 {#list-all-group-issues}

指定されたグループのすべてのイシューをリスト表示します。
...
## すべてのプロジェクトイシューをリスト表示 {#list-all-project-issues}
...
## プロジェクトイシューを取得する {#retrieve-a-project-issue}

プロジェクトの指定されたイシューを取得します。
...
ターゲットプロジェクトで**イシュー**が[無効](../user/project/settings/_index.md#toggle-project-features)になっている場合、`403`応答とメッセージが返されます:
...
不正利用を防ぐため、ユーザーに対して、1分あたりの`Create`リクエストの数を特定の数に制限できます。詳細については、[イシューとエピック作成のレート制限](../administration/settings/rate_limit_on_issues_creation.md)を参照してください。

## イシューを更新 {#update-an-issue}

プロジェクトの指定されたイシューを更新します。このリクエストは、`state_event`パラメータを使用してイシューをクローズまたは再オープンするためにも使用されます。
...
- ユーザーは、GitLab 18.10で[導入された](https://gitlab.com/gitlab-org/gitlab/-/issues/371104)自分が作成したイシューを削除できます。

{{< /history >}}

プランナーまたはオーナーロールを持つユーザーは、任意のイシューを削除できます。他のプロジェクトメンバーは、自分が作成したイシューを削除できます。

指定されたプロジェクトからイシューを削除します。
...
プロジェクト内の指定されたイシューを並べ替えます。[イシューを手動でソート](../user/project/issues/sorting_issue_lists.md#manual-sorting)すると、結果を確認できます。
...
指定されたイシューを別のプロジェクトに移動します。ターゲットプロジェクトがソースプロジェクトである場合、またはユーザーに十分な権限がない場合には、ステータスコードの`400`のエラーメッセージが返されます。
...
指定されたイシューを特定のプロジェクトにクローンします。可能な限り多くのデータがコピーされます。ただし、ターゲットプロジェクトにラベルやマイルストーンなどの同等の条件が含まれている場合に限ります。
...
認証済みユーザーを特定のイシューに購読させて、通知を受け取れるようにします。ユーザーがすでにイシューを購読している場合、ステータスコード`304`が返されます。
...
認証済みユーザーを特定のイシューから購読解除して、通知の受信を停止します。ユーザーがイシューを購読していない場合、ステータスコード`304`が返されます。
...
## イシューのTo-Doアイテムを作成する {#create-a-to-do-item-for-an-issue}

指定されたイシューについて、現在のユーザーのTo-Doアイテムを作成します。そのイシューにユーザーのTo-Doアイテムがすでに存在する場合、ステータスコード`304`が返されます。
...
[`/promote_to`](../user/project/quick_actions.md#promote_to)クイックアクションを含むコメントを追加することで、指定されたイシューをエピックにプロモートします。

詳細については、[イシューをエピックにプロモートする](../user/project/issues/managing_issues.md#promote-an-issue-to-an-epic)を参照してください。
...
| 属性   | 型    | 必須 | 説明                              |
|-------------|---------|----------|------------------------------------------|
| `duration`  | 文字列  | はい      | 人間が読める形式での期間。例: `3h30m`。 |
| `id`        | 整数または文字列 | はい      | プロジェクトのグローバルIDまたは[URLエンコードされたパス](rest/_index.md#namespaced-paths)。      |
| `issue_iid` | 整数 | はい      | プロジェクトイシューの内部ID。     |
...
### イシューのタイムトラッキング統計を取得する {#retrieve-time-tracking-stats-for-an-issue}

指定されたイシューのタイムトラッキング統計を、人間が読める形式（例: `1h30m`）と秒数で取得します。
...
### イシューに関連するすべてのマージリクエストをリスト表示 {#list-all-merge-requests-related-to-an-issue}

指定されたイシューに関連するすべてのマージリクエストをリスト表示します。
...
### マージ時にイシューをクローズするすべてのマージリクエストをリスト表示 {#list-all-merge-requests-that-close-an-issue-on-merge}

マージされたときに指定されたイシューをクローズするすべてのマージリクエストをリスト表示します。
...
## イシューのすべての参加者をリスト表示 {#list-all-participants-in-an-issue}

指定されたイシューの参加者であるすべてのユーザーをリスト表示します。
...
[ノートAPI](notes.md)を使用してコメントとやり取りします。

## イシューのユーザーエージェントの詳細を取得する {#retrieve-user-agent-details-for-an-issue}

管理者のみが利用できます。

指定されたイシューを作成したユーザーのユーザーエージェント文字列とIPアドレスを取得します。スパムの追跡に使用されます。
...
どの状態が設定されたか、誰が設定したか、いつ設定されたかを追跡するには、[リソース状態イベントAPI](resource_state_events.md#issues)を使用します。

## インシデント {#incidents}

以下のリクエストは、[インシデント](../operations/incident_management/incidents.md)でのみ利用可能です。

### インシデントのメトリクス画像をアップロード {#upload-a-metric-image-for-an-incident}

[インシデント](../operations/incident_management/incidents.md)でのみ利用可能です。

指定されたインシデントの**メトリクス**タブに表示するメトリクスチャートのスクリーンショットをアップロードします。画像をアップロードするときに、画像をテキストまたは元のグラフへのリンクに関連付けることができます。URLを追加すると、アップロードされた画像の上にあるハイパーリンクを選択して、元のグラフにアクセスできます。
...
### インシデントのすべてのメトリクス画像をリスト表示 {#list-all-metric-images-for-an-incident}

[インシデント](../operations/incident_management/incidents.md)でのみ利用可能です。

指定されたインシデントの**メトリクス**タブに表示されるメトリクスチャートのすべてのスクリーンショットをリスト表示します。
...
### インシデントのメトリクス画像を更新 {#update-a-metric-image-for-an-incident}

[インシデント](../operations/incident_management/incidents.md)でのみ利用可能です。

インシデントの**メトリクス**タブに表示される、指定されたメトリクス画像の属性を更新します。
...
### インシデントからメトリクス画像を削除 {#delete-a-metric-image-from-an-incident}

[インシデント](../operations/incident_management/incidents.md)でのみ利用可能です。

インシデントの**メトリクス**タブから指定されたメトリクス画像を削除します。
